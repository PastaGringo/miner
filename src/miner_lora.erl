...
-record(state, {
    ...
    last_tmst_us :: integer(), % last concentrator tmst reported by the packet forwarder
    last_mono_us :: integer()  % last local monotonic timestamp taken when packet forwarder reported last tmst
}).
...
handle_call({send, Payload, When, ChannelSelectorFun, DataRate, Power, IPol}, From,
    ...
    case select_gateway(Gateways) of
        ...
        {ok, #gateway{ip=IP, port=Port}} ->
            ...
            AdjustedTmst_us = adjust_tmst(When, PrevTmst_us, PrevMono_us),
...
%% at `Tmst_us`.
adjust_tmst(Tmst_us, PrevTmst_us, PrevMonoTime_us) when Tmst_us >= PrevTmst_us ->
    Tmst_us - PrevTmst_us + PrevMonoTime_us;
adjust_tmst(Tmst_us, PrevTmst_us, PrevMonoTime_us) ->
    %% Because `Tmst_us` is less than the last `tmst` we received from
    %% the packet forwarder, we allow for the possiblity one single
    %% roll over of the clock has occurred, and that `Tmst_us` does
    %% represent a time in the past.
    Tmst_us + ?MAX_TMST_VAL - PrevMonoTime_us + PrevMonoTime_us.
